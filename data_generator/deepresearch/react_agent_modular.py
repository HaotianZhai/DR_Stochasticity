...
def extract_json_obj(text: str) -> Optional[Any]:
    if not text:
        return None
    cleaned = strip_answer_tags(text)
    cleaned = cleaned.strip()
...
REASONING_STRUCTURED_PROMPT = """Based on the current research context, analyze what you know and what you still need to find.
...
- established_facts: array of short factual statements grounded in the context
- open_questions: array of gaps that must be resolved
- next_search_directions: array of concrete search angles or query ideas
- contradictions_or_uncertainties: array of conflicts or weak evidence

Do NOT include any extra text. Wrap the JSON in <reasoning></reasoning> tags."""
...
class ModularReactAgent(FnCallAgent):
    ...
    def _structured_summary_response_format(self) -> Dict[str, Any]:
        return {
            "type": "json_schema",
            "json_schema": {
    ...
    def _llm_intersection_queries(
        ...
        response = self._call_module("query", messages, local_configs, stop_sequences=[])
    ...
    def _parse_summary_payload(self, text: str) -> Dict[str, Any]:
        ...
        if not isinstance(findings, list):
            findings = []
        findings = [f.strip() for f in findings if isinstance(f, str) and f.strip()]
        ...
        return {"findings": findings, "citations": normalized_citations}

    def _majority_vote_texts(
        self,
        runs: List[List[str]],
        similarity_threshold: float = 0.8
    ) -> List[str]:
        clusters: List[Dict[str, Any]] = []
        for run_idx, texts in enumerate(runs):
            for text in texts:
                if not text:
                    continue
                matched = False
                for cluster in clusters:
                    if similarity_ratio(text, cluster["text"]) >= similarity_threshold:
        ...
        return [c["text"] for c in clusters if len(c["runs"]) >= 2]

    def _map_supports(
        self,
        original_findings: List[str],
        original_supports: List[int],
        final_findings: List[str],
        similarity_threshold: float = 0.8
    ) -> List[int]:
        mapped: List[int] = []
        for idx in original_supports:
            original_idx = idx - 1
            if original_idx < 0 or original_idx >= len(original_findings):
                continue
            text = original_findings[original_idx]
            best_i = -1
            best_score = 0.0
            for i, candidate in enumerate(final_findings):
                score = similarity_ratio(text, candidate)
                if score > best_score:
                    best_score = score
                    best_i = i
            if best_i >= 0 and best_score >= similarity_threshold:
                mapped.append(best_i + 1)
    ...
    def call_summarization(self, question: str, search_results: str, run_configs: Dict) -> str:
        """
        Summarization module: Processes and summarizes search results.
        """
        use_structure = self.mitigation_config.get("use_structure", False) or self.mitigation_config.get("use_structure_1", False)
        ...
        if use_structure:
            prompt = (
                ...
                "findings (array of strings) and citations (array of objects with keys: "
                "url, title, supports).\n"
                "Each supports field is a list of 1-based indices into the findings array.\n\n"
        ...
        def run_once() -> str:
            ...
            response_format = self._structured_summary_response_format() if use_structure else None
            response = self._call_module("summarization", messages, local_configs, response_format=response_format)
            if use_structure and "Server error" in response:
                response = self._call_module("summarization", messages, local_configs)
            return response

        if use_consistency:
            responses = [run_once() for _ in range(3)]
            if use_structure:
                parsed_runs = [self._parse_summary_payload(r) for r in responses]
                findings_runs = [r["findings"] for r in parsed_runs]
                majority_findings = self._majority_vote_texts(findings_runs)
                merged_citations: Dict[str, Dict[str, Any]] = {}
                for run in parsed_runs:
                    for citation in run["citations"]:
                        url = citation.get("url", "").strip()
                        if not url:
                            continue
                        supports = self._map_supports(
                            run["findings"],
                            citation.get("supports", []),
                            majority_findings
                        )
                        entry = merged_citations.setdefault(
                            url,
                            {"url": url, "title": citation.get("title", ""), "supports": set()}
                        )
                        if not entry["title"] and citation.get("title"):
                            entry["title"] = citation.get("title")
                        entry["supports"].update(supports)
                citations_list = []
                for entry in merged_citations.values():
                    citations_list.append({
                        "url": entry["url"],
                        "title": entry["title"],
                        "supports": sorted(entry["supports"])
                    })
                summary_payload = {
            ...
            response = run_once()
            if use_structure:
                parsed = self._parse_summary_payload(response)
    ...
    def call_reasoning(self, question: str, context: str, run_configs: Dict) -> str:
        """
        Reasoning module: Generates analysis and inference.
        """
        use_structure = self.mitigation_config.get("use_structure", False) or self.mitigation_config.get("use_structure_2", False)
        if use_structure:
            prompt = REASONING_STRUCTURED_PROMPT.format(
                question=question,
        ...
        response = self._call_module("reasoning", messages, run_configs)
    ...
    def generate_queries(
        ...
        k = max(min_k, initial_k - max(0, current_round - 1))
        local_configs = copy.deepcopy(run_configs)
        ...
        intersected = self._llm_intersection_queries(query_sets, run_configs)
        return intersected or query_sets[0]

    def call_final_report(self, question: str, context: str, run_configs: Dict) -> str:
        prompt = (
        ...
        response = self._call_module("query", messages, run_configs)
        if "<answer>" in response and "</answer>" in response:
            return response.split("<answer>")[1].split("</answer>")[0].strip()
        return response.strip()

    def _ensure_non_empty_report(self, report: Optional[str], question: str, context: str, run_configs: Dict) -> str:
        if report is None:
            return self.call_final_report(question, context, run_configs)
        stripped = str(report).strip()
        if not stripped or stripped.lower() == "none":
            return self.call_final_report(question, context, run_configs)
        return stripped
    
    def call_final_synthesis(self, question: str, context: str, run_configs: Dict) -> str:
        """
        ...
        # Use query module config for synthesis (or could add separate synthesis config)
        return self._call_module(
            "query",  # Reuse query module config
            messages,
            run_configs,
            stop_sequences=[]
        )
    ...
    def _run(self, data: Dict, model: str, **kwargs) -> Dict:
        ...
        while num_calls_available > 0:
            ...
            reasoning_response = self.call_reasoning(question, reasoning_context, run_module_configs)
            ...
            if num_calls_available == 0:
                ...
                final_report = self.call_final_report(question, final_report_context, run_module_configs)
                final_report = self._ensure_non_empty_report(final_report, question, accumulated_context, run_module_configs)
            ...
            if '<answer>' in query_response and '</answer>' in query_response:
                ...
                final_report = self.call_final_report(question, final_report_context, run_module_configs)
                final_report = self._ensure_non_empty_report(final_report, question, final_report_context, run_module_configs)
                ...
                try:
                    ...
                    if tool_name == "search" and self.mitigation_config.get("use_ensemble", False):
                        ...
                        if queries:
                            ...
                            messages.append({
                                "role": "assistant",
                                "content": f"<tool_call>{json.dumps(merged_tool_call, ensure_ascii=False)}</tool_call>"
                            })
                    ...
                    summary = self.call_summarization(question, raw_result, run_module_configs)
